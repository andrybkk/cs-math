...
#proof[
  ...
  *2. Гейне $arrow.r.double$ Коши (от противного):*
  ...
  Мы построили последовательность $x_n arrow.r a$, для которой $f(x_n) arrow.not.r A$. Это противоречиво определению Гейне.
])

<sec-local-limit-props>
...
<sec-infinite-infinitesimal-relation>
...
#proof[
  ...
  Противоречие: предел не может быть одновременно числом $f(c)$ и бесконечностью. Значит, функция ограничена.
])
...
<sec-first-remarkable-consequences>
...
<sec-second-remarkable-consequences>
...
<sec-differentiability-criterion>
...
<sec-differentiability-necessity>
